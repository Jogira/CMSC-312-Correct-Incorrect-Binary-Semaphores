...
struct CSem empty_sem; /* when 0, buffer is empty. Kind of
...
void initializeCSem(struct CSem *flag, int count)
{
	flag -> value = count;
	if (count > 0)
	{
		sem_init(&(flag->gate), 0, 1);
	}
	else
	{
		sem_init(&(flag->gate),0,0);
	}
	sem_init(&(flag->mutex),0,1);
}

void waitC(struct CSem *flag)
{
	sem_wait(&(flag->gate));
	sem_wait(&(flag->mutex));
	flag->value--;
...
void *producer(void *thread_n) {
    ...
    while (i++ < PRODUCER_LOOPS) {
        sleep(rand() % 10);
        value = rand() % 100;
        waitC(&full_sem); // sem=0: wait. sem>0: go and decrement it
        ...
        postC(&empty_sem); // post (increment) emptybuffer semaphore
...
void *consumer(void *thread_n) {
    ...
    while (i++ < PRODUCER_LOOPS) {
        ...
        pthread_mutex_unlock(&buffer_mutex);
        postC(&full_sem); // post (increment) fullbuffer semaphore
